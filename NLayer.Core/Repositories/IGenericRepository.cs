...
namespace NLayer.Core.Repositories
{
    public interface IGenericRepository<T> where T : class
    {
        ...
        // Burada T'den kastımız ilgili entity'i alacak arkasından x.id>5 burdaki ifade de ise true veya false dönecek.
        ...
        // Update veya remove bunların async methodları yok ef core tarafında olmasına da gerek yok çünkü ef core memory'e alıp takip etmiş olduğu bir product'ın sadece state'ini değiştiriyor bu uzun süren bir işlem olmadığı için async yapısı yok yani biz bir şeyi update edebilmek için bir entity vermemiz lazım bu entity zaten memory de ef core tarafından takip ediliyor update dediğimizde sadece o memory de takip etmiş olduğu entitynin state'ini modified olarak değiştiriyor yani uzun süren bir işlem değil ama add memory'e bir data ekliyor burada bir süreç var uzun süren bir işlem olduğu için async'si var ama update ve remove'un ef core tarafında async methodu yok çünkü uzun süren işlemler değil.Async methodu kullanmamamızın sebebi var olan thread'leri bloklamamak için thread'leri daha efektif kullanmak.
        void Update(T entity);
        void Remove(T entity);
        // Aşağıda da bir list almıyoruz bir interface alıyoruz. Nedeni yazılımda mümkün olduğunca soyut nesnelerle çalışmak çok önemli interfaceler gibi abstract classlar gibi bunlar soyut nesnelerdir new anahtar sözcüğü kullanarak nesne örneği almamalıyız mümkün olduğunca soyut nesnelerle çalışmak önemlidir.Çünkü aşağıdaki IEnumerable<T> istediğimiz herhangi bir tipe dönüştürebiliriz. Yani IEnumerable interface'ini implemente etmiş herhangi bir class'a cast yapabiliriz.
